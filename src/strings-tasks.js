...
function formatTime(minutes, seconds) {
  const formatMinutes = minutes.toString(10).padStart(2, '0');
  const formatSeconds = seconds.toString(10).padStart(2, '0');
...
function countVowels(str) {
  const result = str.match(/[aeiouy]/gi);
  if (!result) return 0;
...
function isPalindrome(str) {
  const formatStr = str.match(/[a-z]/gi).join('').toLocaleLowerCase();
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Za-z]/g, (x) => output[input.indexOf(x)]);
}
